<footer style="background-color: #1a2a3a; color: #ffffff; padding: 60px 0 30px; font-family: 'Inter', 'Helvetica Neue', Arial, sans-serif;">
        <div style="max-width: 1280px; margin: 0 auto; padding: 0 40px;">
            <!-- Main Footer Grid -->
            <div
                style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 50px; margin-bottom: 40px;">

                <!-- Brand Column -->
                <div>
                    <div style="display: flex; align-items: center; margin-bottom: 25px;">

                        <span
                            style="font-size: 20px; font-weight: 700; letter-spacing: 0.5px;font-family:Dancing script">MOMENTO</span>
                    </div>
                    <p style="font-size: 14px; line-height: 1.7; color: #a0a8b0; margin-bottom: 25px;">Built for
                        Photographers, By Photographers. The First Platform to Unite Creativity & Tools. </p>
                    <div style="display: flex; gap: 16px;">
                        <a href="#" style="color: #ffffff; text-decoration: none; font-size: 16px;">LinkedIn</a>
                        <a href="#" style="color: #ffffff; text-decoration: none; font-size: 16px;">Twitter</a>
                        <a href="#" style="color: #ffffff; text-decoration: none; font-size: 16px;">Instagram</a>
                    </div>
                </div>

                <!-- Navigation Sections -->
                <div>
                    <h4
                        style="font-size: 14px; font-weight: 600; margin-bottom: 22px; color: #ffffff; text-transform: uppercase; letter-spacing: 1.2px; position: relative; padding-bottom: 8px;">
                        Company
                        <span
                            style="position: absolute; bottom: 0; left: 0; width: 40px; height: 2px; background: #3a86ff;"></span>
                    </h4>
                    <ul style="list-style: none; padding: 0; margin: 0;">
                        <li style="margin-bottom: 14px;"><a href="/momento/about.php"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">About
                                Us</a></li>
                        <li style="margin-bottom: 14px;"><a href="/momento/contact"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">Contact</a>
                        </li>
                    </ul>
                </div>
                <div>
                    <h4
                        style="font-size: 14px; font-weight: 600; margin-bottom: 22px; color: #ffffff; text-transform: uppercase; letter-spacing: 1.2px; position: relative; padding-bottom: 8px;">
                        Resources
                        <span
                            style="position: absolute; bottom: 0; left: 0; width: 40px; height: 2px; background: #3a86ff;"></span>
                    </h4>
                    <ul style="list-style: none; padding: 0; margin: 0;">
                        <li style="margin-bottom: 14px;"><a href="/help-center"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">Help
                                Center</a></li>
                        <li style="margin-bottom: 14px;"><a href="Integration.php"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">Developer
                                API</a></li>
                    </ul>
                </div>

                <div>
                    <h4
                        style="font-size: 14px; font-weight: 600; margin-bottom: 22px; color: #ffffff; text-transform: uppercase; letter-spacing: 1.2px; position: relative; padding-bottom: 8px;">
                        Legal
                        <span
                            style="position: absolute; bottom: 0; left: 0; width: 40px; height: 2px; background: #3a86ff;"></span>
                    </h4>
                    <ul style="list-style: none; padding: 0; margin: 0;">
                        <li style="margin-bottom: 14px;"><a href="/privacy"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">Privacy
                                Policy</a></li>
                        <li style="margin-bottom: 14px;"><a href="termss.php"
                                style="color: #d1d5db; text-decoration: none; font-size: 14px; transition: all 0.3s ease; display: block;">Terms
                                of Service</a></li>
                    </ul>
                </div>
            </div>

            <!-- Divider -->
            <div style="border-top: 1px solid #2a3a4a; margin: 40px 0 30px;"></div>

            <!-- Bottom Footer -->
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;" class="text-center">
                <div style="margin-bottom: 15px;">
                    <p style="font-size: 12px; color: #7f8c8d; margin: 0 0 8px 0;">© 2025 Momento Group, Inc. All rights
                        reserved.</p>
                </div>
            </div>
        </div>
    </footer>