            ...
            <?php endif; ?>
                  ...
                  <div class="stat-card">
                    ...
                    <div class="quick-actions">
                        <button class="btn btn-outline-primary mb-2"onclick = "window.location.href = '/momento/profile.php'">
                            <i class="fas fa-upload me-2"></i> Upload New Work
                        </button>
                        <button class="btn btn-outline-primary mb-2" onclick = "window.location.href = '/momento/chat/chat.php'">
                            <i class="fas fa-envelope me-2"></i> Check Messages
                        </button>
                        <button class="btn btn-outline-primary" onclick = "window.location.href = '/momento/appointment/appointments_inbox.php'">
			...
			<?php require('appointment/calender.php');?>
                ...
                <div class="stat-card mb-4">
					...
					<?php
						...
						$rating = isset($result['total_rate']) ? safe_divide($result['total_rate'],$result['total_reviews']) : 0;
						
						// Round 0.25 and 0.75 to 0.5 for display purposes
						$displayRating = $rating;
						$decimal = $rating - floor($rating);
						if ($decimal >= 0.15 && $decimal < 0.35) {
							$displayRating = floor($rating) + 0.5; // Round 0.25 to 0.5
						} else if ($decimal >= 0.65 && $decimal < 0.85) {
							$displayRating = floor($rating) + 0.5; // Round 0.75 to 0.5
						}
						
						// Calculate stars based on modified display rating
					...
					<h2><?=$rating?> <small class="text-muted">/ 5.0</small></h2>
...
<!------------------------------------------------------------------------------------------------------------->
                    ...
                    <a href="/momento/include/reviews.php" class="btn btn-outline-primary w-100 mt-2">View All Reviews</a>
                </div>
                <!-- Quick Actions -->
